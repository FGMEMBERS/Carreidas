...
var gps_data = props.globals.getNode("/instrumentation/gps[1]",1);
...
var power_knob = func() { #manage POWER knob
    ...
    var light = 0;
    if (isOn > 0 and getprop("instrumentation/gps[1]/serviceable") != 0)
	light = (isOn - 1)/20;
...
var waypointAlert = func { #alert pilot about waypoint approach
    if (mode) { 
	var ttw = getprop("/instrumentation/gps[1]/wp/wp[1]/TTW-sec");
	ttw > -1 or return;
...
var load_bookmarks = func { #load turnpoints
	...
	fgcommand("loadxml", props.Node.new({
	    "filename": file,
	    "targetnode": "/instrumentation/gps[1]/bookmarks"
	}));
	foreach (var c ;props.globals.getNode("/instrumentation/gps[1]/bookmarks").getChildren("bookmark")) n += 1;
    }
    else 
...
var init_gps_props = func {
    ...
    aircraft.light.new("/sim/model/gps[1]/redled", [0.1, 0.1, 0.1, 0.7], "/instrumentation/gps[1]/waypoint-alert");
    aircraft.light.new("/sim/model/gps[1]/greenled", [0.6, 0.3], "/instrumentation/gps[1]/message-alert");
    ...
    setlistener("/instrumentation/gps[1]/serviceable", func {
	if (getprop("/instrumentation/gps[1]/serviceable") == 0)
